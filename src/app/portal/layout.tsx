...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
	...
	const [member, feed, unreadCount, navPins] = await Promise.all([
		...
		features.notifications ? repositories.notifications.listFeed(actor, { limit: 10 }).catch(() => []) : Promise.resolve([]),
		features.notifications ? repositories.notifications.unreadCount(actor).catch(() => 0) : Promise.resolve(0),
	...
	const adminGroups = showAdmin
		? visibleGroups(actor, features).map((group) => ({
